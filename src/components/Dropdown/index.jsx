...
export default function Dropdown({ dropdownName, dropdownData, selectOption }) {
  ...
  return (
    <div className="w-44 font-normal dropdown-container">
      <div
        onClick={() => setToggleDropdown(!toggleDropdown)}
        className={`bg-white w-full p-2 dropdown-header
        flex justify-between items-center border-solid border border-black cursor-pointer`}>
        {selected ? selected : `${dropdownName}`}
        <BiChevronDown size={20} />
      </div>
      {dropdownData?.length > 0 && (
        <div className={`${!toggleDropdown ? 'hidden' : 'absolute w-44'}`}>
          <ul className={'bg-white mt-2 overflow-y-auto max-h-60 border border-black-300 z-20'}>
